 ...
 * Simulates the experience of pressing a matatu horn
 */
...
matatuBox.addEventListener('mouseover', function() {
    ...
    this.querySelector('.matatu-body').style.backgroundColor = randomColor;
...
matatuBox.addEventListener('click', function() {
    ...
    // Add different patterns on click
    if (randomPattern === 1) {
...
routeInput.addEventListener('keyup', function(event) {
 ...
 * Interactive feature #1: Theme switching
 */
...
/**
 * Fare Calculator
 * Interactive feature #2: Calculate matatu fare based on distance
 */
let distance = 0;
...
function calculateFare() {
    let fare = 50; // Base fare for first 5km
    if (distance > 5) {
        fare += (distance - 5) * 10; // KSh 10 per additional km
    }
    fareDisplay.textContent = `KSh ${fare}`;
    
    // Visual feedback based on fare
    if (fare > 100) {
        fareDisplay.style.color = '#e74c3c';
    } else {
        fareDisplay.style.color = '#2ecc71';
    }
}
...
/**
 * Matatu FAQ Section
 * Interactive feature #3: Collapsible content about matatu culture
 */
...
/**
 * Matatu Types Tabs
 * Interactive feature #4: Tab navigation for different matatu types
 */
...
/**
 * Matatu Community Signup Form Validation
 * Custom validation for the newsletter signup form
 */
 ...
 * Validates that location is provided
 */
